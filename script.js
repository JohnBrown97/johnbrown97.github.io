...
function calculateProfit() {
    const saburraCost = parseCurrency(document.getElementById("saburraCost").value);
    const includeSaburra = document.getElementById("includeSaburra").checked;
    ...
    const profit = totalRevenue - (includeSaburra ? saburraCost : 0);
    details += `Total Revenue: ${formatCurrency(totalRevenue)}\n`;
    details += `Saburra Cost: ${formatCurrency(includeSaburra ? saburraCost : 0)}\n`;
    ...
    document.getElementById("resultLabel").innerText = `Profit: ${formatCurrency(Math.abs(profit))}`;
